...
const getXML = async () => {
  ...
  try {
    ...
    const filteredEvent = event.filter(i =>
    i.$.id !== '' &&  i.presenterorge[0] !== '' && i.titlee[0] !== '' && i.pricee[0] !== '' && i.predateE[0] !== '' && i.venueid[0] !==''
    )
      .map(item => ({
    ...
    const filteredLoc = venue.filter(i => locationDict[i.$.id].length >= 3 && i.venuee[0] !== '' && i.latitude[0] !== '' && i.longitude[0] !== '' && i.$.id !== '')
      .map(i => ({
