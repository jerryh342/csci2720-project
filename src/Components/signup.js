...
export const SignUp = () => {
  ...
  const handleSubmit = async (values) => {
    ...
    if (showErr || userNameUsed || wrongPw) {
      resetFormStatus();
      return null;
    }
  ...
  if (role !== "admin") {
    return (
      <div style={{width: "50%", margin: "auto"}}>
        <SuccessPage status="403" title="403" subTitle="Sorry, you are not authorized to access this page." path="/venue" />
      </div>
    );
  }
  return (
        ...
        <div style={{display: "flex", justifyContent: "center", marginTop: 100}}>
          ...
          >
            <Form.Item
              ...
              <Input type="string" name="username" placeholder={userNameUsed ? "user exists" : "input username"} onChange={handleChange} status={userNameUsed ? "error" : ""} prefix={userNameUsed ? <ClockCircleOutlined /> : null} />
            </Form.Item>
            <Form.Item
              labelCol={{
                span: 8,
              ...
              style={{color: "red", textAlign: "left"}}>
              <Input.Password placeholder={wrongPw ? "password not strong enough" : "input password"} name="password" onChange={handleChange} status={wrongPw ? "error" : ""} prefix={wrongPw ? <ClockCircleOutlined /> : null} />
            </Form.Item>
            <Form.Item
              labelCol={{
                span: 8,
              ...
              rules={[{required: true, message: "Password is required"}]}
              style={{color: "red", textAlign: "left"}}>
              <Input.Password placeholder={showErr ? "password not match" : "confirm password"} name="confirmPassword" onChange={handleChange} status={showErr ? "error" : ""} prefix={showErr ? <ClockCircleOutlined /> : null} />
            </Form.Item>
            <Form.Item
              labelCol={{
                span: 8,
            ...
            <Button type="primary" htmlType="submit" disabled={!formData.username || !formData.password || !formData.confirmPassword ? true : false}>
              Register
            </Button>
          </Form>
        </div>
      ) : (
        <SuccessPage status={"success"} path={"/venue"} title={`Successfully create ${userdata.role}: ${userdata.username}`} subTitle={"Thank you for signing up."} />
      )}
    </>
  );
};
